...
resource "google_compute_forwarding_rule" "ms-internal-lb-forwarding-rule-f1" {
  ...
  region                = var.region
  name                  = "${random_pet.pet-prefix.id}-f1-ms-internal-lb"
...
resource "google_compute_health_check" "f1-ms-healthcheck" {
  name                  = "${random_pet.pet-prefix.id}-f1-api-healthcheck"
  http_health_check {
    port                = "3000"
...
resource "google_compute_region_backend_service" "f1-microservice-backend" {
  name          = "${random_pet.pet-prefix.id}-f1-microservice-backend"
  region        = var.region
  backend {
...
resource "google_compute_address" "f1-internal_lb-ip" {
  name         = "${random_pet.pet-prefix.id}-f1-internal-loadbalancer-ip"
